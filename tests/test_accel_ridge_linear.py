...
def test_accel_ridge_linear_smoke_run(
    synthetic_loader_case: dict[str, Path],
) -> None:
    project_root = synthetic_loader_case["project_root"]
    model_config_path = project_root / "configs" / "models" / "accel_ridge_linear.yaml"
    _write_model_config(model_config_path, "outputs/accel_ridge_linear", [1e-6, 1e-3, 1.0])

    data_config = load_data_config(synthetic_loader_case["config_path"])
    model_config = load_accel_ridge_linear_config(model_config_path)
    runs = load_runs(data_config)

    result = train_accel_ridge_linear(runs, data_config, model_config)

    assert result.selected_alpha in model_config.alpha_grid
    assert result.feature_columns == ["acc_x", "acc_y", "acc_z"]
    ...
    assert list(result.coefficient_table["feature"]) == ["acc_x", "acc_y", "acc_z"]
    assert result.coefficient_table["feature_group"].tolist() == ["accel", "accel", "accel"]
...
def test_accel_ridge_linear_recovers_known_linear_mapping(
    synthetic_loader_case: dict[str, Path],
) -> None:
    coefficients = {
        "intercept": -0.5,
    ...
    project_root = synthetic_loader_case["project_root"]
    model_config_path = project_root / "configs" / "models" / "accel_ridge_linear.yaml"
    _write_model_config(model_config_path, "outputs/accel_ridge_linear", [1e-12])
