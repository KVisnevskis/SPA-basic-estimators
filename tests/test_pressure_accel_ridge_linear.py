...
def _write_model_config(path: Path, output_dir: str, alpha_grid: list[float]) -> None:
    payload = {
        "name": "pressure_accel_ridge_linear",
        "fit_intercept": True,
...
def test_pressure_accel_ridge_linear_smoke_run(
    synthetic_loader_case: dict[str, Path],
) -> None:
    project_root = synthetic_loader_case["project_root"]
    model_config_path = (
        project_root / "configs" / "models" / "pressure_accel_ridge_linear.yaml"
    )
    _write_model_config(model_config_path, "outputs/pressure_accel_ridge_linear", [1e-6, 1e-3, 1.0])

    data_config = load_data_config(synthetic_loader_case["config_path"])
    model_config = load_pressure_accel_ridge_linear_config(model_config_path)
    runs = load_runs(data_config)

    result = train_pressure_accel_ridge_linear(runs, data_config, model_config)

    assert result.selected_alpha in model_config.alpha_grid
    assert result.feature_columns == ["pressure", "acc_x", "acc_y", "acc_z"]
    ...
    assert list(result.coefficient_table["feature"]) == ["pressure", "acc_x", "acc_y", "acc_z"]
    assert result.coefficient_table["feature_group"].tolist() == [
        "pressure",
        "accel",
        "accel",
        "accel",
    ...
    with pd.HDFStore(result.all_dataset_predictions_path, mode="r") as store:
        ...
        assert abs(float(per_run["acc_x"].iloc[0]) - 0.0) < 1e-9
        extra_run = store["/predictions/run_extra_1"]
        assert extra_run["split"].unique().tolist() == [UNASSIGNED_SPLIT]


def test_pressure_accel_ridge_linear_recovers_known_linear_mapping(
    synthetic_loader_case: dict[str, Path],
) -> None:
    coefficients = {
        "intercept": 1.0,
    ...
    train_frame["phi"] = phi_values(train_frame)
    ...
    val_frame["phi"] = phi_values(val_frame)
    ...
    test_frame["phi"] = phi_values(test_frame)
    ...
    _write_model_config(model_config_path, "outputs/pressure_accel_ridge_linear", [1e-12])
