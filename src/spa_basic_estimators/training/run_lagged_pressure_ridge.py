...
from spa_basic_estimators.estimators.lagged_pressure_ridge import (
    run_lagged_pressure_ridge,
)


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the lagged pressure-only ridge baseline."
    )
    parser.add_argument(
        "--data-config",
    ...
    parser.add_argument(
        "--model-config",
        default="configs/models/lagged_pressure_ridge.yaml",
        help="Path to the lagged pressure-only ridge model config YAML.",
    )
    return parser
...
def main() -> None:
    ...
    result = run_lagged_pressure_ridge(
