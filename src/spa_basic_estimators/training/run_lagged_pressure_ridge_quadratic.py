...
from spa_basic_estimators.estimators.lagged_pressure_ridge_quadratic import (
    run_lagged_pressure_ridge_quadratic,
)


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the lagged pressure-only quadratic ridge baseline."
    )
    parser.add_argument(
        "--data-config",
    ...
    parser.add_argument(
        "--model-config",
        default="configs/models/lagged_pressure_ridge_quadratic.yaml",
        help="Path to the lagged pressure-only quadratic ridge model config YAML.",
    )
    return parser
...
def main() -> None:
    ...
    result = run_lagged_pressure_ridge_quadratic(
