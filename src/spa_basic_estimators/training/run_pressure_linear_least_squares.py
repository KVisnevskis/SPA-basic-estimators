...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the pressure-only ordinary least-squares baseline."
    )
    parser.add_argument(
        "--data-config",
    ...
    parser.add_argument(
        "--model-config",
        default="configs/models/pressure_linear_least_squares.yaml",
        help="Path to the pressure-only least-squares model config YAML.",
    )
    return parser
...
def main() -> None:
    ...
    result = run_pressure_linear_least_squares(
