...
from spa_basic_estimators.estimators.accel_ridge_polynomial_search import (
    run_accel_ridge_polynomial_search,
)


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the accel-only polynomial-search ridge baseline."
    )
    parser.add_argument(
        "--data-config",
    ...
    parser.add_argument(
        "--model-config",
        default="configs/models/accel_ridge_polynomial_search.yaml",
        help="Path to the accel-only polynomial-search ridge model config YAML.",
    )
    return parser
...
def main() -> None:
    ...
    result = run_accel_ridge_polynomial_search(
