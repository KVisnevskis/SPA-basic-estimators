...
AccelLinearLeastSquaresConfig = LinearLeastSquaresConfig
AccelLinearLeastSquaresResult = LinearLeastSquaresTrainingResult


def load_accel_linear_least_squares_config(path: str | Path) -> AccelLinearLeastSquaresConfig:
    common_config = load_linear_least_squares_config(
        path,
        default_name="accel_linear_least_squares",
        default_output_dir="outputs/accel_linear_least_squares",
    )
    return AccelLinearLeastSquaresConfig(**common_config.__dict__)
...
def train_accel_linear_least_squares(
    runs: Mapping[str, pd.DataFrame],
    data_config: DataConfig,
    estimator_config: AccelLinearLeastSquaresConfig,
    ...
    coefficient_table = pd.DataFrame(
        {
            ...
            "feature_group": ["accel"] * len(dataset.feature_columns),
    ...
    return AccelLinearLeastSquaresResult(
...
def run_accel_linear_least_squares(
    data_config_path: str | Path = "configs/data.yaml",
    model_config_path: str | Path = "configs/models/accel_linear_least_squares.yaml",
) -> AccelLinearLeastSquaresResult:
    data_config = load_data_config(data_config_path)
    estimator_config = load_accel_linear_least_squares_config(model_config_path)
    runs = load_runs(data_config)
    return train_accel_linear_least_squares(runs, data_config, estimator_config)
