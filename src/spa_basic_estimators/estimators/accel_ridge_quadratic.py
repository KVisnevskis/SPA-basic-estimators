...
from spa_basic_estimators.estimators.pressure_ridge_common import (
    ...
    PressureRidgeConfig,
    PressureRidgeResult,
    build_accel_only_dataset as build_accel_only_dataset_common,
...
class AccelRidgeQuadraticConfig(PressureRidgeConfig):
    degree: int


AccelRidgeQuadraticResult = PressureRidgeResult


def load_accel_ridge_quadratic_config(path: str | Path) -> AccelRidgeQuadraticConfig:
    common_config = load_pressure_ridge_config(
        path,
        default_name="accel_ridge_quadratic",
        default_output_dir="outputs/accel_ridge_quadratic",
    )
    raw = load_yaml(Path(path).resolve())
    degree = int(raw.get("degree", 2))
    if degree != 2:
        raise ValueError("Accel-only quadratic ridge currently supports degree=2 only")

    return AccelRidgeQuadraticConfig(
...
def build_accel_only_dataset(
    ...
    return build_accel_only_dataset_common(runs, data_config)


def train_accel_ridge_quadratic(
    runs: Mapping[str, pd.DataFrame],
    data_config: DataConfig,
    estimator_config: AccelRidgeQuadraticConfig,
) -> AccelRidgeQuadraticResult:
    dataset = build_accel_only_dataset_common(runs, data_config)
    ...
    if best_alpha is None:
        raise ValueError("Alpha grid is empty; cannot train accel-only quadratic ridge model")
    ...
    coefficient_table = pd.DataFrame(
        {
            ...
            "feature_group": ["accel"] * len(feature_names),
    ...
    return AccelRidgeQuadraticResult(
...
def run_accel_ridge_quadratic(
    data_config_path: str | Path = "configs/data.yaml",
    model_config_path: str | Path = "configs/models/accel_ridge_quadratic.yaml",
) -> AccelRidgeQuadraticResult:
    data_config = load_data_config(data_config_path)
    estimator_config = load_accel_ridge_quadratic_config(model_config_path)
    runs = load_runs(data_config)
    return train_accel_ridge_quadratic(runs, data_config, estimator_config)
