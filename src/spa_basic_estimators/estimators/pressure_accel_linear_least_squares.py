...
PressureAccelLinearLeastSquaresConfig = LinearLeastSquaresConfig
PressureAccelLinearLeastSquaresResult = LinearLeastSquaresTrainingResult


def load_pressure_accel_linear_least_squares_config(
    path: str | Path,
) -> PressureAccelLinearLeastSquaresConfig:
    common_config = load_linear_least_squares_config(
        path,
        default_name="pressure_accel_linear_least_squares",
        default_output_dir="outputs/pressure_accel_linear_least_squares",
    )
    return PressureAccelLinearLeastSquaresConfig(**common_config.__dict__)
...
def train_pressure_accel_linear_least_squares(
    runs: Mapping[str, pd.DataFrame],
    data_config: DataConfig,
    estimator_config: PressureAccelLinearLeastSquaresConfig,
    ...
    return PressureAccelLinearLeastSquaresResult(
...
def run_pressure_accel_linear_least_squares(
    data_config_path: str | Path = "configs/data.yaml",
    model_config_path: str | Path = "configs/models/pressure_accel_linear_least_squares.yaml",
) -> PressureAccelLinearLeastSquaresResult:
    data_config = load_data_config(data_config_path)
    estimator_config = load_pressure_accel_linear_least_squares_config(model_config_path)
    runs = load_runs(data_config)
    return train_pressure_accel_linear_least_squares(runs, data_config, estimator_config)
