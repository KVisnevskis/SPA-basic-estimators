...
class LaggedPressureRidgeConfig(RidgeModelConfig):
    lag_grid: list[int]


@dataclass(frozen=True)
class LaggedPressureRidgeResult(RidgeTrainingResult):
    selected_lag: int
    raw_feature_columns: list[str]


def load_lagged_pressure_ridge_config(path: str | Path) -> LaggedPressureRidgeConfig:
    common_config = load_ridge_model_config(
        path,
        default_name="lagged_pressure_ridge",
        default_output_dir="outputs/lagged_pressure_ridge",
    )
...
def train_lagged_pressure_ridge(
    runs: Mapping[str, pd.DataFrame],
    data_config: DataConfig,
    estimator_config: LaggedPressureRidgeConfig,
) -> LaggedPressureRidgeResult:
    raw_feature_columns = pressure_feature_columns(data_config)
    ...
    if best_alpha is None or best_lag is None:
        raise ValueError("Lagged pressure ridge could not select a best lag/alpha pair")
    ...
    return LaggedPressureRidgeResult(
...
def run_lagged_pressure_ridge(
    data_config_path: str | Path = "configs/data.yaml",
    model_config_path: str | Path = "configs/models/lagged_pressure_ridge.yaml",
) -> LaggedPressureRidgeResult:
    data_config = load_data_config(data_config_path)
    estimator_config = load_lagged_pressure_ridge_config(model_config_path)
    runs = load_runs(data_config)
    return train_lagged_pressure_ridge(runs, data_config, estimator_config)
