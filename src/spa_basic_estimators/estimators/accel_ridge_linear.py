...
AccelRidgeLinearConfig = PressureRidgeConfig
AccelRidgeLinearResult = PressureRidgeResult


def load_accel_ridge_linear_config(path: str | Path) -> AccelRidgeLinearConfig:
    common_config = load_pressure_ridge_config(
        path,
        default_name="accel_ridge_linear",
        default_output_dir="outputs/accel_ridge_linear",
    )
    return AccelRidgeLinearConfig(**common_config.__dict__)
...
def train_accel_ridge_linear(
    runs: Mapping[str, pd.DataFrame],
    data_config: DataConfig,
    estimator_config: AccelRidgeLinearConfig,
    ...
    if best_alpha is None:
        raise ValueError("Alpha grid is empty; cannot train accel-only ridge model")
...
def run_accel_ridge_linear(
    data_config_path: str | Path = "configs/data.yaml",
    model_config_path: str | Path = "configs/models/accel_ridge_linear.yaml",
) -> AccelRidgeLinearResult:
    data_config = load_data_config(data_config_path)
    estimator_config = load_accel_ridge_linear_config(model_config_path)
    runs = load_runs(data_config)
    return train_accel_ridge_linear(runs, data_config, estimator_config)
