...
class LaggedAccelRidgeQuadraticConfig(RidgeModelConfig):
    lag_grid: list[int]
    degree: int


@dataclass(frozen=True)
class LaggedAccelRidgeQuadraticResult(RidgeTrainingResult):
    selected_lag: int
    raw_feature_columns: list[str]


def load_lagged_accel_ridge_quadratic_config(
    path: str | Path,
) -> LaggedAccelRidgeQuadraticConfig:
    common_config = load_ridge_model_config(
        path,
        default_name="lagged_accel_ridge_quadratic",
        default_output_dir="outputs/lagged_accel_ridge_quadratic",
    )
    ...
    if not lag_grid:
        raise ValueError("Lagged accel quadratic ridge requires at least one lag length")
    ...
    if degree != 2:
        raise ValueError("Lagged accel quadratic ridge currently supports degree=2 only")

    return LaggedAccelRidgeQuadraticConfig(
...
def train_lagged_accel_ridge_quadratic(
    runs: Mapping[str, pd.DataFrame],
    data_config: DataConfig,
    estimator_config: LaggedAccelRidgeQuadraticConfig,
) -> LaggedAccelRidgeQuadraticResult:
    raw_feature_columns = accel_feature_columns(data_config)
    ...
    if best_alpha is None or best_lag is None:
        raise ValueError("Lagged accel quadratic ridge could not select a best lag/alpha pair")
    ...
    return LaggedAccelRidgeQuadraticResult(
...
def run_lagged_accel_ridge_quadratic(
    data_config_path: str | Path = "configs/data.yaml",
    model_config_path: str | Path = "configs/models/lagged_accel_ridge_quadratic.yaml",
) -> LaggedAccelRidgeQuadraticResult:
    data_config = load_data_config(data_config_path)
    estimator_config = load_lagged_accel_ridge_quadratic_config(model_config_path)
    runs = load_runs(data_config)
    return train_lagged_accel_ridge_quadratic(runs, data_config, estimator_config)
