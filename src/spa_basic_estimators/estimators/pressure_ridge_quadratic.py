...
def train_pressure_ridge_quadratic(
    runs: Mapping[str, pd.DataFrame],
    data_config: DataConfig,
    estimator_config: PressureRidgeQuadraticConfig,
    ...
    if best_alpha is None:
        raise ValueError("Alpha grid is empty; cannot train pressure-only quadratic ridge model")
    ...
    return PressureRidgeQuadraticResult(
...
def run_pressure_ridge_quadratic(
    data_config_path: str | Path = "configs/data.yaml",
    model_config_path: str | Path = "configs/models/pressure_ridge_quadratic.yaml",
) -> PressureRidgeQuadraticResult:
    data_config = load_data_config(data_config_path)
    estimator_config = load_pressure_ridge_quadratic_config(model_config_path)
    runs = load_runs(data_config)
    return train_pressure_ridge_quadratic(runs, data_config, estimator_config)
